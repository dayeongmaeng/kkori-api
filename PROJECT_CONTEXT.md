...
- 앱 출시일: 2026-07-08
- 배포 전 필수였던 운영 DB 마이그레이션 4건 모두 적용 완료 (회원 탈퇴, 체중 모름, user_oauth_token, DailyLog 확장 필드)
...
- ✅ Phase C+: 8080 외부 노출 차단 (127.0.0.1 바인딩) + prod 프로파일 docker-compose 적용 (2026-05-31)
...
- ✅ Phase D+: 회원 탈퇴 API 서버 구현 완료 + 클라이언트 UI 연동 완료 (운영 DB 마이그레이션 적용 완료)
...
- ✅ Phase E+: 기록탭 사진 저장 시점 변경 - 기록 등록 + 사진 업로드 한 번에 처리하는 API 추가 완료 (2026-07-07)
...
- Pet — soft delete 적용
  ...
  - name, species, gender, breed, birthDate, birthDateUnknown, adoptionDate, weightKg, weightKgUnknown, neutered, medicalNotes, photoBase64
  ...
  - `weightKgUnknown=true`이면 `weightKg` null 허용. 운영 DB 적용 완료: `pet-weight-kg-unknown-migration.sql`
- DailyPhoto — soft delete 적용
  - `petId`, `caregiverId`, date, caption, mediumUrl, thumbnailUrl
  - unique(`petId`, `date`)
- DailyLog — soft delete 적용
  - 기본: `petId`, `caregiverId`, date, meal, water, walkMinutes, pooCondition, urineColor, urineAmount, condition, weightKg, memo
  - 확장 메모: `mealNote`, `walkNote`, `pooNote`, `urineNote`, `waterNote`, `playMinutes`, `playNote`, `vomitCount`, `vomitNote`
  - `urineAmount` enum: LITTLE, NORMAL, MUCH
  - unique(`petId`, `date`)
  - 운영 DB 적용: `daily-log-note-fields-migration.sql`
...
- 공개 API: auth login/refresh, health, swagger, 공유 조회, `/oauth/kakao`.
- Kakao authorization code redirect는 정적 페이지 `/oauth/kakao` (`static/oauth/kakao.html`)를 거쳐 `kkori://oauth/kakao` 딥링크로 포워딩한다. JWT 필터 제외 대상 (2026-06-18)
...
앱 출시(2026-07-08) 전에 운영 DB에 적용 완료됨.
...
- `OAUTH_TOKEN_ENCRYPTION_KEY`: AES-256-GCM 기반 OAuth 토큰 암호화 키. 운영 환경변수 필수. 운영 적용 완료.
...
5. 실제 Google/Kakao OAuth 실기기 로그인 QA + Google revoke 실기기 QA
